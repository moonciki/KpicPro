...
k.width||722,k.height||440,"tmpVedio"+g,null,!1));d.execCommand("inserthtml",h.join(""),!0);h=this.selection.getRange();g=0;for(l=b.length;g<l;g++)k=this.document.getElementById("tmpVedio"+g),e.removeAttributes(k,"id"),h.selectNode(k).select(),d.execCommand("imagefloat",b[g].align)},queryCommandState:function(){var a=d.selection.getRange().getClosedNode();return a&&"edui-faked-video"==a.className?1:0}}};(function(){var f=UE.UETable=function(a){this.table=a;this.indexTable=[];this.selectedTds=[];this.cellsRange=
