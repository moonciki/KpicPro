        ...
        if(data != null && data != ""){
            ...
            for(key in data){
                addhtml += "<div class='album_list'><span class=\"badge pull-right\" style='cursor: pointer; background-color: #ff7575;' id=\"sctj\"><span class='glyphicon glyphicon-remove-sign'></span> 删除</span>" +
                        "<span class=\"badge pull-right\" style='margin-right:10px;cursor: pointer; background-color: #64A600;' onclick=\"fbtj('"+data[key].title+"', "+data[key].id+", "+data[key].createTime+", '"+data[key].music+"','"+data[key].imageUrl+"', '"+data[key].msg+"', "+data[key].picNum+")\"><span class='glyphicon glyphicon-send'></span> 发布</span>"+
...
function fbtj(title, id, createTime, music, imageUrl, msg, picNum){
    $(".pic_big").show();
    $("#title").val(title);
    $("#id").val(id);
