    ...
    $.post("/user/focus/subjects", {'userId':userId, 'page' : page}, function(data){
        ...
        for(key in data){
            ...
            var addhtml = "<div class=\"panel panel-default\" style='border: solid "+data[key].color+" 1px'><div class=\"panel-body\">" +
                ...
                "<span class=\"badge pull-right\" style = \"margin-top:15px;font-size:18px;cursor:pointer; background-color:"+data[key].color+"\" onclick='tzht("+data[key].id+")'><b>进入圈子 >></b></span>"+
            "</div></div>"
            $("#gzht").append(addhtml);
